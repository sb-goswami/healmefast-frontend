...
export const SignupForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await authApi.post('/auth/signup', { full_name: fullName, email, password });
      setSuccess(true);
      setTimeout(() => login(response.data.access_token, response.data.user), 1500);
    } catch (err: any) {
  ...
  if (success) {
    return (
      <motion.div 
        ...
        className="w-full max-w-md p-8 rounded-2xl bg-white/10 backdrop-blur-xl border border-white/20 shadow-2xl text-center"
      >
        ...
        <p className="text-blue-100/60">Redirecting you to login...</p>
      </motion.div>
    );
  }
  ...
  return (
    ...
    >
      <div className="text-center mb-8">
        ...
        <p className="text-blue-100/60">Join the HealmeFast community</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="space-y-1">
          <label className="text-sm font-medium text-white/80 block ml-1">Full Name</label>
        ...
        <button 
          type="submit" 
          disabled={loading}
          className="w-full bg-blue-600 hover:bg-blue-500 text-white font-semibold py-3 mt-4 rounded-xl shadow-lg shadow-blue-600/20 flex items-center justify-center gap-2 group transition-all disabled:opacity-50"
        >
          {loading ? (
