...
export const ForgotPasswordForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // In this mock version, we might show the token for testing
      if (response.data.mock_token) {
        console.log("Mock Reset Token:", response.data.mock_token);
      }
    } catch (err: any) {
  ...
  return (
    ...
    >
      <div className="mb-8">
        <Link href="/auth/login" className="text-blue-400 hover:text-blue-300 flex items-center gap-2 text-sm mb-6 transition-colors">
          <ArrowLeft className="w-4 h-4" /> Back to login
        </Link>
        <h2 className="text-3xl font-bold text-white mb-2">Reset Password</h2>
        <p className="text-blue-100/60">We'll send you a link to reset your password</p>
      </div>
